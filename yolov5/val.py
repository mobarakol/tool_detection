...
def calc_iou(
    ...
    if len(targets) > 0 and len(predictions) > 0:
        ...
        # gt_count += np.sum(np.array(gt_boxes) <= 0, axis=1).item()
        # pred_count += np.sum(np.array(pred_boxes) <= 0, axis=1).item()
        ...
        final_iou_class_list = np.divide(np.array(iou_class_list), np.array(counter_list)) 
